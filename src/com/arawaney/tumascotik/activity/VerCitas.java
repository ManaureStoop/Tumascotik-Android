...
import com.arawaney.tumascotik.adapter.ItemListBaseAdapter;
import com.arawaney.tumascotik.db.CitationDB;
import com.arawaney.tumascotik.dialog.ListItemDialog;
		...
		flagclick = false;
		 ...
		 listcitas.setOnItemClickListener(new OnItemClickListener() {
	        	...
	        	public void onItemClick(AdapterView<?> a, View v, int position, long id) { 
	            	...
	            	DialogFragment newFragment = new ListItemDialog(obj_itemDetails.getName() , obj_itemDetails.getMotive(),obj_itemDetails.getInitialDate(),obj_itemDetails.getFinalDate(),getIntent().getStringExtra("seleccion"));
					newFragment.show(getSupportFragmentManager(), "itemdetails");
	        	}  }
	        });
	  ...
	  if(getIntent().getStringExtra("seleccion").equals("ACEPTADAS")){
	   	//Example TODO
	     	try{
			    cur = db.getAccepted();
		     	...
		     	item_details.setItemDescription("Cita para : "+cur.getString(MOTIVE_INDEX));
		     	
		     	fechai =new Date(Long.parseLong(cur.getString(STARTDATE_INDEX)));
		     	fechaf =new Date(Long.parseLong(cur.getString(ENDDATE_INDEX)));			
		     	item_details.setTime("D�a : "+android.text.format.DateFormat.format("EEEE, dd", fechai)+System.getProperty("line.separator").toString()+android.text.format.DateFormat.format("         MMMM, yyyy", fechai)+System.getProperty("line.separator").toString()+"Hora : "+android.text.format.DateFormat.format("h:mmaa", fechai)+"-"+android.text.format.DateFormat.format("h:mmaa", fechaf));
		     	item_details.setName(cur.getString(PET_INDEX));	 	
		     	item_details.setRace(cur.getString(RACE_INDEX));
		     // Have to save this data in order tu delete it from database and from Parse cloud if necesary
   	...
   	//Example TODO
	     	try{
		     	...
		     	// Have o save this data in order tu delete it from database and from Parse cloud if necesary
		     	item_details.setMotive(cur.getString(MOTIVE_INDEX));  	
		     	item_details.setInitalDate(fechai);
		     	item_details.setFinalDate(fechaf);
